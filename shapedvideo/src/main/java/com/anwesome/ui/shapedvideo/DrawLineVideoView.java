...
public class DrawLineVideoView extends ShapedVideoView {
    ...
    private class Line {
        ...
        public void computeInnerPoints(float endX,float endY) {
            float theta = (endX == startX)?90:Math.abs(endY-startY)/(Math.abs(endX-startX));
            ...
            while(true) {
                x+=linePointRadius*Math.cos(theta*Math.PI/180);
                y+=linePointRadius*Math.sin(theta*Math.PI/180);
